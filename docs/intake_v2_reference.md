...
cat.to_yaml_file("ocean_skill/catalogs/woa.catalog.yaml")

# consumer side
cat = intake.from_yaml_file("ocean_skill/catalogs/woa.catalog.yaml")
...
`ocean_skill/catalogs/woa.catalog.yaml` (WOA23 nitrate/phosphate/silicate/oxygen, annual
1°, via OPeNDAP) — generated by the pattern above. `catalogs/ROMS.yaml` (combined ROMS
file, `XArrayDatasetReader` + `engine: netcdf4`, project-local/gitignored since it embeds
a local path) is another. Both are v2.
