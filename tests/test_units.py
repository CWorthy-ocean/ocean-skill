...
# -- a value's own _qc/_raw tier is preferred, not excluded like a flag ----------------


def _tiered(base: str, *, qc=1.0, raw=0.9, units="PSU", flag=True):
    """A mooring-CSV-shaped dataset: <base>_raw/_qc (with units) plus a bare _flag."""
    data = {
        f"{base}_qc": (("time",), np.full(3, qc), {"units": units}),
        f"{base}_raw": (("time",), np.full(3, raw), {"units": units}),
    }
    if flag:
        data[f"{base}_flag"] = (("time",), np.zeros(3))  # no units -- a real flag
    return xr.Dataset(data, coords={"time": [1, 2, 3]})


def test_qc_tier_is_preferred_over_raw_when_both_carry_a_unit():
    """`Salinity_qc[PSU]` is the quality-controlled *value*, not a flag -- even though

    "qc" is also one of the tokens ``is_qc_name`` watches for; the unit is what tells
    the two conventions apart (see units.py's ``_tiered_candidates`` docstring).
    """
    ds = _tiered("Salinity", qc=33.5, raw=33.4)
    found = u.find_variable(ds, "salinity")
    assert found is not None
    assert found.name == "Salinity_qc"
    assert float(found.isel(time=0)) == 33.5


def test_raw_tier_is_used_when_qc_is_absent():
    ds = _tiered("Salinity", raw=33.4).drop_vars("Salinity_qc")
    found = u.find_variable(ds, "salinity")
    assert found is not None and found.name == "Salinity_raw"


def test_flag_sibling_is_never_returned_for_the_tiered_measurement():
    ds = _tiered("Salinity")
    assert u.find_variable(ds, "salinity").name != "Salinity_flag"
    assert u.is_qc_name("Salinity_flag")  # excluded through the ordinary flag path
...
def test_qc_tier_preference_applies_to_every_mooring_variable(base, nickname):
    """The same qc-over-raw preference generalizes across the whole mooring vocabulary."""
    ds = _tiered(base, qc=1.0, raw=2.0)
    found = u.find_variable(ds, nickname)
    assert found is not None
    assert found.name == f"{base}_qc"


def test_a_bare_qc_tier_with_no_units_is_still_treated_as_a_flag():
    """No unit -> the ERDDAP convention wins: `_qc` with nothing attached is a flag."""
    ds = xr.Dataset(
        {"Salinity_raw": (("time",), np.full(3, 33.4), {"units": "PSU"})},
        coords={"time": [1, 2, 3]},
    )
    ds["Salinity_qc"] = (("time",), np.ones(3))  # no units attribute
    found = u.find_variable(ds, "salinity")
    assert found is not None and found.name == "Salinity_raw"
